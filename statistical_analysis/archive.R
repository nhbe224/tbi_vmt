...
# Draft 2 Specs without remote day pct and employment entropy --------------
## Overall  -------------------------------
opsr_model_final_eq <- work_arr | ln_vmt_weekly ~ 
  ...
  ## Interactions
  hh_income_under50:post_covid + hh_income100to150:post_covid +
  hh_income150to200:post_covid + 
  hh_income_200plus:post_covid + hh_income_undisclosed:post_covid +
  no_bach_plus:post_covid +
  nonWhite:post_covid + nonmale:post_covid + nonmale_kids_1plus:post_covid |
  ...
  num_adults +
  nonmale_kids_1plus +
  #nonmale_kids_0 + male_kids_1plus +
  ## Year
  year2021 + year2023 + nonmale_kids_1plus:year2021 + nonmale_kids_1plus:year2023 | 
...
rebound_nonwork_increase_eq <- work_arr | ln_nonwork_tour_vmt_weekly ~ 
  ...
  hh_income_200plus:post_covid + hh_income_undisclosed:post_covid +
  no_bach_plus:post_covid +
  nonWhite:post_covid + nonmale:post_covid |
...
aip_work_vmt_factual <- predict(rebound_work_decrease, group = 1, type = "unlog-response", w = vmt_weekly_df$person_weight, vmt_weekly_model_mat)
aip_work_vmt_if_hybrid <- predict(rebound_work_decrease, group = 1, counterfact = 2, type = "unlog-response", w = vmt_weekly_df$person_weight, vmt_weekly_model_mat)
vmt_weekly_df$aip_work_vmt_if_hybrid <- aip_work_vmt_if_hybrid
vmt_weekly_df$aip_work_vmt_factual <- aip_work_vmt_factual

vmt_weekly_df_aip <- vmt_weekly_df %>%
  mutate(te =  aip_work_vmt_if_hybrid - aip_work_vmt_factual) %>%
  relocate(person_id, person_weight, work_tour_vmt_weekly, aip_work_vmt_factual, aip_work_vmt_if_hybrid, te) %>%
  filter(work_arr == "Always In-Person") 
weighted.mean(vmt_weekly_df_aip$te, vmt_weekly_df_aip$person_weight)

vmt_files <- readRDS("../datacleaning/outputs/vmt_files.RData")
vmt_weekly_df <- vmt_files[["vmt_weekly_df"]]

h_work_vmt_factual <- predict(rebound_work_decrease, group = 2, type = "unlog-response", w = vmt_weekly_df$person_weight, vmt_weekly_model_mat)
h_work_vmt_if_aip <- predict(rebound_work_decrease, group = 2, counterfact = 1, type = "unlog-response", w = vmt_weekly_df$person_weight, vmt_weekly_model_mat)

vmt_weekly_df$h_work_vmt_factual <- h_work_vmt_factual
vmt_weekly_df$h_work_vmt_if_aip <- h_work_vmt_if_aip

vmt_weekly_df_h <- vmt_weekly_df %>%
  mutate(te =  h_work_vmt_factual - h_work_vmt_if_aip) %>%
  relocate(person_id, person_weight, work_tour_vmt_weekly, h_work_vmt_if_aip, h_work_vmt_factual, te) %>%
  filter(work_arr == "Hybrid") 

weighted.mean(vmt_weekly_df_h$te, vmt_weekly_df_h$person_weight)

hist(vmt_weekly_df_h$te)


### Get summary stats
round(wpct(vmt_weekly_df_h$age_group, vmt_weekly_df_h$person_weight) * 100)
round(wpct(vmt_weekly_df_aip$age_group, vmt_weekly_df_aip$person_weight) * 100)

round(wpct(vmt_weekly_df_h$gender, vmt_weekly_df_h$person_weight) * 100)
round(wpct(vmt_weekly_df_aip$gender, vmt_weekly_df_aip$person_weight) * 100)

round(wpct(vmt_weekly_df_h$num_kids, vmt_weekly_df_h$person_weight) * 100)
round(wpct(vmt_weekly_df_aip$num_kids, vmt_weekly_df_aip$person_weight) * 100)

round(wpct(vmt_weekly_df_h$auto_sufficiency, vmt_weekly_df_h$person_weight) * 100)
round(wpct(vmt_weekly_df_aip$auto_sufficiency, vmt_weekly_df_aip$person_weight) * 100)

round(wpct(vmt_weekly_df_h$employment, vmt_weekly_df_h$person_weight) * 100)
round(wpct(vmt_weekly_df_aip$employment, vmt_weekly_df_aip$person_weight) * 100)

round(wpct(vmt_weekly_df_h$num_hybrid_or_remote, vmt_weekly_df_h$person_weight) * 100)
round(wpct(vmt_weekly_df_aip$num_hybrid_or_remote, vmt_weekly_df_aip$person_weight) * 100)

res_den_df <- vmt_weekly_df %>%
  filter(work_arr != "Always Remote") %>%
  select(person_id, person_weight, work_arr, ln_res_den)

ggplot(res_den_df, aes(x = ln_res_den, weight = person_weight, fill = work_arr)) +
  geom_histogram(position = "identity", alpha = 0.5, bins = 30) +
  labs(title = "Housing Unit Density by Work Arrangement",
       x = "Housing Unit Density",
       y = "Weighted Count")

intersection_den_df <- vmt_weekly_df %>%
  filter(work_arr != "Always Remote") %>%
  select(person_id, person_weight, work_arr, ln_intersection_den)

ggplot(intersection_den_df, aes(x = ln_intersection_den, weight = person_weight, fill = work_arr)) +
  geom_histogram(position = "identity", alpha = 0.5, bins = 30) +
  labs(title = "Intersection Density by Work Arrangement",
       x = "Intersection Density",
       y = "Weighted Count")

emp_ent_df <- vmt_weekly_df %>%
  filter(work_arr != "Always Remote") %>%
  select(person_id, person_weight, work_arr, emp8_ent)

ggplot(emp_ent_df, aes(x = emp8_ent, weight = person_weight, fill = work_arr)) +
  geom_histogram(position = "identity", alpha = 0.5, bins = 30) +
  labs(title = "Employment Entropy by Work Arrangement",
       x = "Employment Entropy",
       y = "Weighted Count")

round(wpct(vmt_weekly_df_h$survey_year, vmt_weekly_df_h$person_weight) * 100)
round(wpct(vmt_weekly_df_aip$survey_year, vmt_weekly_df_aip$person_weight) * 100)

## Women with kids always in-person 2021
round(wpct(vmt_weekly_model_mat[vmt_weekly_model_mat$work_arr == 1 & vmt_weekly_model_mat$survey_year == 2021, ]$nonmale_kids_1plus, vmt_weekly_model_mat[vmt_weekly_model_mat$work_arr == 1 & vmt_weekly_model_mat$survey_year == 2021, ]$person_weight) * 100)
## Women with kids hybrid 2021
round(wpct(vmt_weekly_model_mat[vmt_weekly_model_mat$work_arr == 2 & vmt_weekly_model_mat$survey_year == 2021, ]$nonmale_kids_1plus, vmt_weekly_model_mat[vmt_weekly_model_mat$work_arr == 2 & vmt_weekly_model_mat$survey_year == 2021, ]$person_weight) * 100)

## Women with kids always in-person 2023
round(wpct(vmt_weekly_model_mat[vmt_weekly_model_mat$work_arr == 1 & vmt_weekly_model_mat$survey_year == 2023, ]$nonmale_kids_1plus, vmt_weekly_model_mat[vmt_weekly_model_mat$work_arr == 1 & vmt_weekly_model_mat$survey_year == 2023, ]$person_weight) * 100)
## Women with kids hybrid 2023
round(wpct(vmt_weekly_model_mat[vmt_weekly_model_mat$work_arr == 2 & vmt_weekly_model_mat$survey_year == 2023, ]$nonmale_kids_1plus, vmt_weekly_model_mat[vmt_weekly_model_mat$work_arr == 2 & vmt_weekly_model_mat$survey_year == 2023, ]$person_weight) * 100)




# Test: what if all outcome variables are also in selection? --------------
rebound_work_decrease_test_eq <- work_arr | ln_work_tour_vmt_weekly ~ 
  # Factors that affect work arrangement
  ## Age
  age18to34 + age55plus +
  ## Education
...
## Estimation
rebound_work_decrease_test <- opsr(rebound_work_decrease_test_eq, vmt_weekly_model_mat, printLevel = 0)
## Treatment effects 
rebound_work_decrease_test_te <- opsr_te(rebound_work_decrease_test, type = "unlog-response", weights = vmt_weekly_model_mat$person_weight)

summary(rebound_work_decrease_test)
print(rebound_work_decrease_test_te)
texreg(rebound_work_decrease_test, single.row = T, stars = c(0.001, 0.01, 0.05, 0.1), symbol = "\\dagger")




comp_df1 <- vmt_weekly_df_h %>%
  filter(auto_sufficiency == 1)

weighted.mean(comp_df1$te, comp_df1$person_weight)

comp_df2 <- vmt_weekly_df_h %>%
  filter(auto_sufficiency == 2)

weighted.mean(comp_df2$te, comp_df2$person_weight)

vmt_weekly_df %>%
  filter(work_arr != "Always Remote") %>%
  ggplot(aes(x = work_tour_vmt_weekly, weight = person_weight, fill = work_arr)) +
  geom_histogram(position = "identity", alpha = 0.5, bins = 30) +
  labs(title = "Observed Weekly Work Tour VMT",
       x = "Work Tour VMT",
       y = "Weighted Count")

ggplot(vmt_weekly_df_aip, aes(x = aip_work_vmt_if_hybrid, weight = person_weight, fill = work_arr)) +
  geom_histogram(position = "identity", alpha = 0.5, bins = 30) +
  labs(title = "Predicted Weekly Work Tour VMT",
       x = "Work Tour VMT",
       y = "Weighted Count")

ggplot(vmt_weekly_df_h, aes(x = h_work_vmt_factual, weight = person_weight, fill = work_arr)) +
  geom_histogram(position = "identity", alpha = 0.5, bins = 30) +
  labs(title = "Predicted Weekly Work Tour VMT",
       x = "Work Tour VMT",
       y = "Weighted Count")

ggplot(vmt_weekly_df_aip, aes(x = aip_work_vmt_factual, weight = person_weight, fill = work_arr)) +
  geom_histogram(position = "identity", alpha = 0.5, bins = 30) +
  labs(title = "Predicted Weekly Work Tour VMT",
       x = "Work Tour VMT",
       y = "Weighted Count")

ggplot(vmt_weekly_df_h, aes(x = h_work_vmt_if_aip, weight = person_weight, fill = work_arr)) +
  geom_histogram(position = "identity", alpha = 0.5, bins = 30) +
  labs(title = "Predicted Weekly Work Tour VMT",
       x = "Work Tour VMT",
       y = "Weighted Count")
...
# Test: Same selection model, only a constant -----------------------------
rebound_work_decrease_eq2 <- work_arr | ln_work_tour_vmt_weekly ~ 
  # Factors that affect work arrangement
  ## Age
  age18to34 + age55plus +
  ## Education
...
## Estimation
rebound_work_decrease2 <- opsr(rebound_work_decrease_eq2, vmt_weekly_model_mat, printLevel = 0)
## Treatment effects 
rebound_work_decrease_te2 <- opsr_te(rebound_work_decrease2, type = "unlog-response", weights = vmt_weekly_model_mat$person_weight)

summary(rebound_work_decrease2)
print(rebound_work_decrease_te2)
texreg(rebound_work_decrease2, single.row = T, stars = c(0.001, 0.01, 0.05, 0.1), symbol = "\\dagger")




# OLD WORK -------------------------------------------------------------------
## With income in selection and outcome model
opsr_model_test_eq <- work_arr | ln_vmt_weekly ~ 
  # Factors that affect work arrangement
  ## Age
  age18to34 + age55plus +
  ## Education
...
## Estimation
opsr_model_test <- opsr(opsr_model_test_eq, vmt_weekly_model_mat, printLevel = 0)

## Treatment effects 
opsr_model_test_te <- opsr_te(opsr_model_test, type = "unlog-response", weights = vmt_weekly_model_mat$person_weight)

## Inference
summary(opsr_model_test)
texreg(opsr_model_test, single.row = T, stars = c(0.001, 0.01, 0.05, 0.1), symbol = "\\dagger")
print(opsr_model_test_te)
...
ols_model1 <- lm(ln_vmt_weekly ~ work_arr + survey_year:work_arr + age_group + 
                   ...
                   jobs_per_hh + emp8_ent + intersection_den + survey_year, 
...
texreg(opsr_model1_rep, single.row = T, stars = c(0.001, 0.01, 0.05, 0.1), symbol = "\\dagger")
...
# print(rho2)
# rho3 <- c(0.131235 - 1.96*(0.343881), 0.131235 + 1.96*(0.219460))
# print(rho3)
...
## OPSR Model 2 ------------------------------------------------------------
#### Adding income seems to really mess with the results.


#### Add column: women with children by year (binary columns)
opsr_model2_eq <- work_arr | ln_vmt_weekly ~ 
  # Factors that affect work arrangement
  ## Education
  no_bach_plus + 
  ## Gender
...
## Estimation
opsr_model2 <- opsr(opsr_model2_eq, vmt_weekly_model_mat, printLevel = 0)

## Treatment effects 
opsr_model2_te <- opsr_te(opsr_model2, type = "unlog-response", weights = vmt_weekly_model_mat$person_weight)

## Inference
summary(opsr_model2)
#texreg(opsr_model2, single.row = T)
print(opsr_model2_te)
#screenreg(opsr_model2, single.row = T)

## Calculate average treatment effect on treated
ar_vmt_observed <- weighted.mean(predict(opsr_model2, group = 3, type = "unlog-response"), w = vmt_weekly_model_mat$person_weight, na.rm = T)
ar_vmt_counterfact <- weighted.mean(predict(opsr_model2, group = 3, counterfact = 1,  type = "unlog-response"), w = vmt_weekly_model_mat$person_weight, na.rm = T)
ar_vmt_observed
ar_vmt_counterfact
ar_vmt_observed - ar_vmt_counterfact
(ar_vmt_observed - ar_vmt_counterfact) / ar_vmt_counterfact

hybrid_vmt_observed <- weighted.mean(predict(opsr_model2, group = 2, type = "unlog-response"), w = vmt_weekly_model_mat$person_weight, na.rm = T)
hybrid_vmt_counterfact <- weighted.mean(predict(opsr_model2, group = 2, counterfact = 1,  type = "unlog-response"), w = vmt_weekly_model_mat$person_weight, na.rm = T)
hybrid_vmt_observed
hybrid_vmt_counterfact
hybrid_vmt_observed - hybrid_vmt_counterfact
(hybrid_vmt_observed - hybrid_vmt_counterfact) / hybrid_vmt_counterfact
...
## OPSR Model ----------------------------------
opsr_model3_eq <- work_arr | ln_vmt_weekly ~ 
  # Factors that affect work arrangement
  ## Education
  no_bach_plus + 
  ## Gender
...
## Estimation
opsr_model3 <- opsr(opsr_model3_eq, vmt_weekly_model_mat, printLevel = 0)

## Treatment effects 
opsr_model3_te <- opsr_te(opsr_model3, type = "unlog-response", weights = vmt_weekly_model_mat$person_weight)

## Inference
summary(opsr_model3)
#texreg(opsr_model2, single.row = T)
print(opsr_model3_te)

## OPSR Model (No income in outcome model) ----------------------------------
opsr_model4_eq <- work_arr | ln_vmt_weekly ~ 
  # Factors that affect work arrangement
  ## Education
  no_bach_plus + 
  ## Gender
...
## Estimation
opsr_model4 <- opsr(opsr_model4_eq, vmt_weekly_model_mat, printLevel = 0)

## Treatment effects 
opsr_model4_te <- opsr_te(opsr_model4, type = "unlog-response", weights = vmt_weekly_model_mat$person_weight)

## Inference
summary(opsr_model4)
#texreg(opsr_model2, single.row = T)
print(opsr_model4_te)


# OPSR Model (Age in selection model) ----------------------------------
opsr_model5_eq <- work_arr | ln_vmt_weekly ~ 
  # Factors that affect work arrangement
  ## Age
  age18to34 + age55plus +
  ## Education
...
## Estimation
opsr_model5 <- opsr(opsr_model5_eq, vmt_weekly_model_mat, printLevel = 0)

## Treatment effects 
opsr_model5_te <- opsr_te(opsr_model5, type = "unlog-response", weights = vmt_weekly_model_mat$person_weight)

## Inference
summary(opsr_model5)
#texreg(opsr_model2, single.row = T)
print(opsr_model5_te)

# OPSR Model (Age in selection model, income in outcome) ----------------------------------
opsr_model6_eq <- work_arr | ln_vmt_weekly ~ 
  # Factors that affect work arrangement
  ## Age
  age18to34 + age55plus +
  ## Education
...
## Estimation
opsr_model6 <- opsr(opsr_model6_eq, vmt_weekly_model_mat, printLevel = 0)

## Treatment effects 
opsr_model6_te <- opsr_te(opsr_model6, type = "unlog-response", weights = vmt_weekly_model_mat$person_weight)

## Inference
summary(opsr_model6)
#texreg(opsr_model2, single.row = T)
print(opsr_model6_te)
...
## Estimation
opsr_model7 <- opsr(opsr_model7_eq, vmt_weekly_model_mat, printLevel = 0)

## Treatment effects 
opsr_model7_te <- opsr_te(opsr_model7, type = "unlog-response", weights = vmt_weekly_model_mat$person_weight)

## Inference
summary(opsr_model7)
#texreg(opsr_model2, single.row = T)
print(opsr_model7_te)
...
## Estimation
opsr_model8 <- opsr(opsr_model8_eq, vmt_weekly_model_mat, printLevel = 0)

## Treatment effects 
opsr_model8_te <- opsr_te(opsr_model8, type = "unlog-response", weights = vmt_weekly_model_mat$person_weight)

## Inference
summary(opsr_model8)
#texreg(opsr_model2, single.row = T)
print(opsr_model8_te)

ar_vmt_observed <- weighted.mean(predict(opsr_model8, group = 3, type = "unlog-response"), w = vmt_weekly_model_mat$person_weight, na.rm = T)
ar_vmt_counterfact <- weighted.mean(predict(opsr_model8, group = 3, counterfact = 1,  type = "unlog-response"), w = vmt_weekly_model_mat$person_weight, na.rm = T)
ar_vmt_observed
ar_vmt_counterfact
ar_vmt_observed - ar_vmt_counterfact
(ar_vmt_observed - ar_vmt_counterfact) / ar_vmt_counterfact

hybrid_vmt_observed <- weighted.mean(predict(opsr_model8, group = 2, type = "unlog-response"), w = vmt_weekly_model_mat$person_weight, na.rm = T)
hybrid_vmt_counterfact <- weighted.mean(predict(opsr_model8, group = 2, counterfact = 1,  type = "unlog-response"), w = vmt_weekly_model_mat$person_weight, na.rm = T)
hybrid_vmt_observed
hybrid_vmt_counterfact
hybrid_vmt_observed - hybrid_vmt_counterfact
(hybrid_vmt_observed - hybrid_vmt_counterfact) / hybrid_vmt_counterfact
...
# OPSR Model 8 with Non-Work ----------------------------------------------
opsr_model8_nonwork_eq <- work_arr | ln_nonwork_tour_vmt_weekly ~ 
  # Factors that affect work arrangement
  ## Age
  age18to34 + age55plus +
  ## Education
...
## Estimation
opsr_model8_nonwork <- opsr(opsr_model8_nonwork_eq, vmt_weekly_model_mat, printLevel = 0)

## Treatment effects 
opsr_model8_nonwork_te <- opsr_te(opsr_model8_nonwork, type = "unlog-response", weights = vmt_weekly_model_mat$person_weight)

## Inference
summary(opsr_model8_nonwork)
#texreg(opsr_model2, single.row = T)
print(opsr_model8_nonwork_te)


# OPSR Model 8 with Work ----------------------------------------------
opsr_model8_work_eq <- work_arr | ln_work_tour_vmt_weekly ~ 
  # Factors that affect work arrangement
  ## Age
  age18to34 + age55plus +
  ## Education
...
## Estimation
opsr_model8_work <- opsr(opsr_model8_work_eq, vmt_weekly_model_mat, printLevel = 0)

## Treatment effects 
opsr_model8_work_te <- opsr_te(opsr_model8_work, type = "unlog-response", weights = vmt_weekly_model_mat$person_weight)

## Inference
summary(opsr_model8_work)
#texreg(opsr_model2, single.row = T)
print(opsr_model8_work_te)
...
# OPSR Model (2019)  -------------------------------------------------
opsr_model2019_eq <- work_arr | ln_vmt_weekly ~ 
  # Factors that affect work arrangement
  ## Education
  no_bach_plus + 
  ## Household Income
...
## Estimation
opsr_model2019 <- opsr(opsr_model2019_eq, vmt_weekly_model_mat[vmt_weekly_model_mat[, "survey_year"] == 2019, ], printLevel = 0)
## Treatment effects 
opsr_model2019_te <- opsr_te(opsr_model2019, type = "unlog-response", weights = vmt_weekly_model_mat[vmt_weekly_model_mat[, "survey_year"] == 2019, ]$person_weight)
## Inference
summary(opsr_model2019)
texreg(opsr_model2019, single.row = T)
print(opsr_model2019_te)
...
# OPSR Model (2021)  -------------------------------------------------
opsr_model2021_eq <- work_arr | ln_vmt_weekly ~ 
  # Factors that affect work arrangement
  ## Household Income
  hh_income_under50 + hh_income100to150 + hh_income150to200 + 
  hh_income_200plus + hh_income_undisclosed +
...
## Treatment effects 
opsr_model2021_te <- opsr_te(opsr_model2021, type = "unlog-response", weights = vmt_weekly_model_mat[vmt_weekly_model_mat[, "survey_year"] == 2021, ]$person_weight)
## Inference
summary(opsr_model2021)
texreg(opsr_model2021, single.row = T)
print(opsr_model2021_te)

predict(opsr_model2021, group = 1, type = "unlog-response")
texreg::screenreg(opsr_model2021, digits = 3, single.row = T)

# OPSR Model (2023)  -------------------------------------------------
opsr_model2023_eq <- work_arr | ln_vmt_weekly ~ 
  # Factors that affect work arrangement
  ## Household Income
  hh_income_under50 + hh_income100to150 + hh_income150to200 + 
  hh_income_200plus + hh_income_undisclosed +
...
## Estimation
opsr_model2023 <- opsr(opsr_model2023, vmt_weekly_model_mat[vmt_weekly_model_mat[, "survey_year"] == 2023, ], printLevel = 0)
## Treatment effects 
opsr_model2023_te <- opsr_te(opsr_model2023, type = "unlog-response", weights = vmt_weekly_model_mat[vmt_weekly_model_mat[, "survey_year"] == 2023, ]$person_weight)
## Inference
summary(opsr_model2023)
texreg(opsr_model2023, single.row = T)
print(opsr_model2023_te)

predict(opsr_model2023, group = 1, type = "unlog-response")

texreg::screenreg(opsr_model2023, digits = 3, single.row = T)

## Check treatment effects for each model
print(opsr_model1_rep_te)
print(opsr_model2019_te) # 2019
print(opsr_model2021_te) # 2021
print(opsr_model2023_te) # 2023
...
disp_model_v3 <- plm(ln_nonwork_tour_vmt_today ~ work_arr_day + n_other_remote_days +
                       ...
                       jobs_per_hh + emp8_ent + intersection_den + 
                       survey_year, 
...
# OPSR Replication --------------------------------------------------------
data(telework_data)
f <- twing_status | vmd_ln ~  edu_2 + edu_3 + hhincome_2 + hhincome_3 + flex_work + work_fulltime + 
  twing_feasibility + att_proactivemode + att_procarowning + att_wif + 
  att_proteamwork + att_tw_effective_teamwork + att_tw_enthusiasm + + att_tw_location_flex | 
